...
class MacroTradingEnv(gym.Env):
    ...
    def _create_observation_space(self):
        ...
        macro_dim = self.macro_data.shape[1]
        technical_dim = self.n_assets * 3  # RSI, SMA_ratio, volatility per asset
        
        total_dim = market_data_dim + portfolio_dim + technical_dim + macro_dim
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if self.macro_data is not None:
            current_idx = self.lookback_window + self.current_step
            macro_values = self.macro_data.iloc[current_idx]
            obs_parts.append(macro_values.values)
